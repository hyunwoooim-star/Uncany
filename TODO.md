...
### 1. Database 마이그레이션 적용 ⚠️ **최우선**

**왜 필요한가**:
- 법률 준수 코드는 구현했지만, DB 스키마가 적용되지 않아 동작하지 않음
- 재직증명서 자동 삭제, 동의 날짜 추적 기능이 DB 마이그레이션에 의존

**적용할 파일**:
1. `supabase/migrations/004_add_document_auto_delete.sql`
2. `supabase/migrations/005_add_consent_tracking.sql`
